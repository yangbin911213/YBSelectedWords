...
+(CFIndex)parserIndexWithPoint:(CGPoint)point frameRef:(CTFrameRef)frameRef
{
    ...
    if (lineCount) {
        ...
        for (int i = 0; i<lineCount; i++) {
            ...
            CGRect lineFrame = CGRectMake(baselineOrigin.x, CGRectGetHeight(bounds)-baselineOrigin.y-ascent, lineWidth, ascent+descent+linegap+[ReadConfig shareInstance].lineSpace);    //没有转换坐标系左下角为坐标原点 字体高度为上行高度加下行高度
    ...
    if (lineCount) {
        ...
        for (int i = 0; i<lineCount; i++) {
            ...
            if (CGRectContainsPoint(lineFrame,point)){
                ...
                // frame加上左边距和上边距
                rect = CGRectMake(origins[i].x+xStart + 12,baselineOrigin.y-descent + 7.5,fabs(xStart-xEnd), ascent+descent);
    ...
    if (lineCount) {
        ...
        for (int i = 0; i<lineCount; i++){
            ...
            if (drawRange.length) {
                ...
                xEnd = CTLineGetOffsetForStringIndex(line, drawRange.location+drawRange.length, NULL);
                // frame加上左边距和上边距
                CGRect rect = CGRectMake(xStart + 12, baselineOrigin.y-descent + 7.5, fabs(xStart-xEnd), ascent+descent);
                if (rect.size.width ==0 || rect.size.height == 0) {
                    continue;
                }
